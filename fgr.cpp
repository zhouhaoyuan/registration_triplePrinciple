...
void CApp::setPointCloud(const std::vector<Points> &input)
{
	pointcloud_.clear();
	pointcloud_ = input;
}
void CApp::setPointCloudfeature(const std::vector<Feature> &input)
{
	features_.clear();
	features_ = input;
}
...
void CApp::NormalizePoints()
{
	...
	for (int i = 0; i < num; ++i)
	{
		...
		int npti = pointcloud_[i].size();//每一组的点云数
		...
		{
			Eigen::Vector3f p(pointcloud_[i][ii](0), pointcloud_[i][ii](1), pointcloud_[i][ii](2));
			float temp = p.norm();//求各点到质心的距离 // because we extract mean in the previous stage.
			if (temp > max_scale)
				max_scale = temp;//获取最大的距离
		}

		if (max_scale > scale)//获取所有点云组中最大的距离
			scale = max_scale;
	}
